  ...
  const fetchEvents = async (isRefresh = false) => {
    ...
    try {
      ...
      if (Array.isArray(eventsData) && eventsData.length > 0) {
        ...
        // If for some reason eventsData is empty, set empty state
        console.log("No events found in response");
        setEvents([]);
        setGroupedEvents([]);
        
        if (!error) {
          setError("No events found. Please try refreshing or contact support.");
        }
      }
    } catch (err: any) {
      ...
      setEvents([]);
      setGroupedEvents([]);
      setError(err.message || "Failed to load events. Please try refreshing or contact support.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
